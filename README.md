...
[![Docker pulls](https://img.shields.io/docker/pulls/shesek/eznode.svg?color=blueviolet)](https://hub.docker.com/r/shesek/eznode)
...
- [🎁 Packages](#-packages)
  - [bitcoind](#bitcoind) &middot; [bwt](#bwt) &middot; [btc-rpc-explorer](#btc-rpc-explorer) &middot; [specter](#specter)
  - [tor](#tor) &middot; [dropbear](#dropbear) &middot; [nginx](#nginx) &middot; [letsencrypt](#letsencrypt)
...
You can access the services remotely using Tor onion services, SSH port tunneling or SSL. Below is a summary of the pros of cons for each approach.
...
Bitcoin Core full node. Pruned by default.
...
```bash
$ docker run -v ~/.bitcoin:/bitcoin:ro --add-host host.docker.internal:host-gateway -it ... eznode/eznode
```

The `--add-host` option makes the host's address discoverable within the container. Instructions for modifying bitcoind's `rpcbind`/`rpcallowip` config will be shown on startup. If you're running into trouble with Docker's virtual networking, you can try with [`--net host`](https://docs.docker.com/network/host/) (this should ideally be avoided).
...
[Bitcoin Wallet Tracker](https://bwt.dev/), a personal wallet tracker that watches your wallet's activity, available as an Electrum RPC server and an [HTTP API](https://github.com/bwt-dev/bwt#http-api).
...
Onion services provide strong encryption and mutual authentication using the `.onion` hostname as a shared secret. It does not require setting up port forwarding.
...
To setup, set `SSL=1`, publish the SSL port and make sure to enable token authentication:
...
```bash
$ alias ez='docker exec -it ez'
$ alias bitcoin-cli='ez bitcoin-cli'
```

With these in place, you can use `bitcoin-cli` as you normally would and `ez` to issue other commands on the container. For example, `ez bash` to start a new shell.
...
To run the docker container as a background daemon and have it automatically run on start-up, replace `-it --rm` with `-d --restart unless-stopped`. For example:
...
```bash
$ docker exec ez backup > ez-backup.tar.gz
```

> `backup` is a shortcut for `tar czvhf - /important`. This won't work with the `ez` alias, because `docker exec` needs to be run without `-it`.
